...
FIXTURES = ROOT / "chapters" / "chapter-01" / "fixtures"


def run_validator(*paths: Path) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        [sys.executable, "-m", "harness_lab.validators.chapter01", "validate", *map(str, paths), "--json"],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result = subprocess.run(
                [
                    sys.executable,
                    "-m",
                    "harness_lab.validators.chapter01",
                    "validate",
                    str(positive),
                    "--json",
                    "--summary-file",
